...
const ShowDrinkScreen = ({route, navigation}) => {
    ...
    let moveUp = scrollY.interpolate({
        inputRange:[-1,0,75],
        outputRange:[0,0, -50]
    })
        ...
        <View style={styles.container}>
            ...
            {!isLoading ?
                <View style={styles.list_container}>
                    <TouchableOpacity disabled={!isAvailable} onPress={() => initToggle()}
                                      style={styles.star_container}>
                        <AntDesign color={"#e67bec"} style={styles.star} size={25} name={`${starOn}`}/>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.back_container} onPress={() => {
                        navigation.goBack()
                    }}>
                    ...
                    >
                        ...
                        <Animated.Image source={{uri: cocktail[0].strDrinkThumb}}
                        ...
                        <View style={styles.grid_one}>
                            ...
                            <Text style={styles.text}><Text style={{
                                fontWeight: "bold",
                                color: "#e67bec"
                            }}>Category: </Text>{cocktail[0].strCategory}</Text>
                            ...
                            {cocktail[0].strIBA ? <View>
                                <Text style={styles.text}>
                                    <Text
                                        style={{fontWeight: "bold", color: "#e67bec"}}>IBA: </Text>{cocktail[0].strIBA}
                                </Text>
                                <View style={styles.lineBreak}/>
                            </View> : null}
                            ...
                            <Text style={styles.text}><Text
                                style={{fontWeight: "bold", color: "#e67bec"}}>Type: </Text>{cocktail[0].strAlcoholic}
                            </Text>
                            ...
                            <Text style={styles.text}><Text
                                style={{fontWeight: "bold", color: "#e67bec"}}>Glass: </Text>{cocktail[0].strGlass}
                            </Text>
                        ...
                        <View style={styles.txt_container}>
                            <Text style={styles.text}>
                                <Text style={{
                                    fontWeight: "bold",
                                    color: "#e67bec"
                                }}>Instructions:</Text> {cocktail[0].strInstructions}
                            </Text>
                        </View>
                        <View style={styles.txt_container}>
                            ...
                            {ingredients.map((item, index) => (
                                <View key={index} style={styles.ingredient_container}>
                                    <Text key={index} style={styles.text_ingredients}><Text
                                        style={{color: "#e67bec"}}>{item.ment}</Text> {item.name}</Text>
                                </View>
                            ))}
                        </View>
                    </Animated.ScrollView>
...
const styles = StyleSheet.create({
    ...
    image: {
        width: 150,
        height: 150,
        borderRadius: (Platform.OS === 'ios'?100:5),
